...
public class SentMultipleVideos extends RecyclerView.ViewHolder {
    View view;
    TextView textView;
    private String DOT_SEPRATOR = " \u25CF ";
    public SentMultipleVideos(@NonNull View itemView) {
        super(itemView);
        view = itemView;
    }
    ...
    public void initalize(Context context,Message message) {
        ...
        messageTime.setText(message.getSharedFiles().size()+" Videos"+DOT_SEPRATOR+LayoutService.getSize(size)+DOT_SEPRATOR+LayoutService.getFormatedDate("hh:mm aa", message.getSentAt()));
